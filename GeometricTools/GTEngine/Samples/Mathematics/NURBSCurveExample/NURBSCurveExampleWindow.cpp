...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"NURBSCurveExampleWindow", 0, 0, 256, 256);
    auto window = TheWindowSystem.Create<NURBSCurveExampleWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<NURBSCurveExampleWindow>(window);
    return 0;
}

NURBSCurveExampleWindow::NURBSCurveExampleWindow(Parameters& parameters)
    :
    Window2(parameters),
    mSize(mXSize),
...
void NURBSCurveExampleWindow::OnDisplay()
{
...
bool NURBSCurveExampleWindow::OnCharPress(unsigned char key, int x, int y)
{
    switch (key)
    {
    case 'g':
...
void NURBSCurveExampleWindow::DoSimulation1()
{
    mSimTime += mSimDelta;
...
void NURBSCurveExampleWindow::DoSimulation2()
{
    mSimTime += mSimDelta;
...
void NURBSCurveExampleWindow::InitialConfiguration()
{
    mSimTime = 0.0f;
    mSpline = nullptr;
    mCircle = nullptr;
...
void NURBSCurveExampleWindow::NextConfiguration()
{
    mTargets.clear();
