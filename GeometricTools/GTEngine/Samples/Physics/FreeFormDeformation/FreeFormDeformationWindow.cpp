...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"FreeFormDeformationWindow", 0, 0, 640, 480);
    auto window = TheWindowSystem.Create<FreeFormDeformationWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<FreeFormDeformationWindow>(window);
    return 0;
}

FreeFormDeformationWindow::FreeFormDeformationWindow(Parameters& parameters)
    :
    Window3(parameters),
...
void FreeFormDeformationWindow::OnIdle()
{
    mTimer.Measure();
...
bool FreeFormDeformationWindow::OnCharPress(unsigned char key, int x, int y)
{
    switch (key)
    {
...
bool FreeFormDeformationWindow::OnMouseClick(MouseButton button, MouseState state,
...
bool FreeFormDeformationWindow::OnMouseMotion(MouseButton button, int x, int y,
...
bool FreeFormDeformationWindow::SetEnvironment()
{
...
void FreeFormDeformationWindow::CreateScene()
{
...
void FreeFormDeformationWindow::CreateBSplineVolume()
{
...
void FreeFormDeformationWindow::CreateSegments()
{
...
void FreeFormDeformationWindow::CreateBoxes()
{
...
void FreeFormDeformationWindow::UpdateMesh()
{
    auto vbuffer = mMesh->GetVertexBuffer();
    unsigned int const numVertices = vbuffer->GetNumElements();
    Vertex* vertices = vbuffer->Get<Vertex>();
...
void FreeFormDeformationWindow::UpdateSegments()
{
...
void FreeFormDeformationWindow::UpdateBoxes()
{
    size_t bound = static_cast<size_t>(mQuantity);
    Array3<Vector3<float>> control(bound, bound, bound,
        const_cast<Vector3<float>*>(mVolume->GetControls()));
...
void FreeFormDeformationWindow::DoRandomControlPoints()
{
...
void FreeFormDeformationWindow::OnMouseDown(int x, int y)
{
    std::shared_ptr<ConstantColorEffect> effect;
...
void FreeFormDeformationWindow::OnMouseMove(int x, int y)
{
