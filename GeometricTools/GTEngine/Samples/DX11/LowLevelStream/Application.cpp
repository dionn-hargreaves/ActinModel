...
Application::~Application()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        if (mConstantBuffer[i])
        {
            mConstantBuffer[i]->Release();
        }
    }
    ...
    mTrackBallDown(false)
{
    ...
    ComputeViewMatrix();
    ComputeWorldMatrix(0);
    ComputeWorldMatrix(1);
}
...
#else
    std::wstring path = L"_Output\\v140\\";
#endif
...
bool Application::CreateVertexBuffers()
{
    mNumVertices = 3;
    mVertexOffset = 0;

    // Create vertex buffer for position.
    float position[9];  // 9 = 3 * mNumVertices, (x,y,z)
    position[0] = 0.0f;
    position[1] = 0.0f;
    position[2] = 0.0f;
    position[3] = 1.0f;
    ...
    vbDesc.ByteWidth = 9 * sizeof(float);
    ...
    HRESULT hr = mDevice->CreateBuffer(&vbDesc, &data, &mVertexBuffer[0]);
    if (FAILED(hr))
    {
        return false;
    }

    // Create vertex buffer for color0.
    float color0[12];  // 12 = 4 * mNumVertices, (r,g,b,a)
    color0[0] = 1.0f;  // bright red
    color0[1] = 0.0f;
    color0[2] = 0.0f;
    color0[3] = 1.0f;
    color0[4] = 0.66f;  // medium red
    color0[5] = 0.0f;
    color0[6] = 0.0f;
    color0[7] = 1.0f;
    color0[8] = 0.33f;  // dark red
    color0[9] = 0.0f;
    color0[10] = 0.0f;
    color0[11] = 1.0f;

    vbDesc.ByteWidth = 12 * sizeof(float);
    vbDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    vbDesc.MiscFlags = 0;
    vbDesc.StructureByteStride = 0;
    vbDesc.Usage = D3D11_USAGE_IMMUTABLE;
    ...
    data.SysMemPitch = 0;
    data.SysMemSlicePitch = 0;
    hr = mDevice->CreateBuffer(&vbDesc, &data, &mVertexBuffer[1]);
    if (FAILED(hr))
    {
        return false;
    }

    // Create vertex buffer for color1.
    float color1[12];  // 12 = 4 * mNumVertices, (r,g,b,a)
    color1[0] = 0.0f;  // bright green
    color1[1] = 1.0f;
    color1[2] = 0.0f;
    color1[3] = 1.0f;
    color1[4] = 0.00f;  // medium green
    color1[5] = 0.66f;
    color1[6] = 0.0f;
    color1[7] = 1.0f;
    color1[8] = 0.0f;  // dark green
    color1[9] = 0.33f;
    color1[10] = 0.0f;
    color1[11] = 1.0f;
    ...
    data.SysMemPitch = 0;
    data.SysMemSlicePitch = 0;
    hr = mDevice->CreateBuffer(&vbDesc, &data, &mVertexBuffer[2]);
    if (FAILED(hr))
    {
        return false;
    }
...
{
    // Create an input layout with 2 input streams (position and color).
    UINT const numElements = 2;
...
bool Application::CreateConstantBuffers()
{
    ...
    cbDesc.ByteWidth = 4 * 4 * sizeof(float);  // to store the 4x4 WVP matrix
    ...
    for (int i = 0; i < 2; ++i)
    {
        HRESULT hr = mDevice->CreateBuffer(&cbDesc, nullptr, &mConstantBuffer[i]);
        if (FAILED(hr))
        {
            return false;
        }
...
void Application::Draw()
{
    ...
    UINT numInstances = 0;
    ID3D11Buffer* cbuffers[1] = { nullptr };
    unsigned int cbBindPoint = 0;  // SimpleVShader.vs_5_0.txt shows a bind point of 0.
    ID3D11Buffer* vbuffers[2] = { nullptr, nullptr };
    UINT strides[2] =
    {
        static_cast<UINT>(3 * sizeof(float)),  // stride for positions
        static_cast<UINT>(4 * sizeof(float))   // stride for colors
    };
    UINT offsets[2] = { (UINT)mVertexOffset, (UINT)mVertexOffset };

    // Set the vertex shader.  This is shared by the two triangles.
    mImmediate->VSSetShader(mVertexShader, instances, numInstances);

    // Set the pixel shader.  This is shared by the two triangles.
    mImmediate->PSSetShader(mPixelShader, instances, numInstances);

    // Set the input layout for the vertex buffer and vertex shader.  This is
    ...
    // Set up and draw the first triangle.
    cbuffers[0] = mConstantBuffer[0];
    vbuffers[0] = mVertexBuffer[0];
    vbuffers[1] = mVertexBuffer[1];
    mImmediate->VSSetConstantBuffers(cbBindPoint, 1, cbuffers);
    mImmediate->IASetVertexBuffers(0, 2, vbuffers, strides, offsets);
    mImmediate->Draw(mNumVertices, mVertexOffset);

    // Set up and draw the second triangle.
    cbuffers[0] = mConstantBuffer[1];
    vbuffers[0] = mVertexBuffer[0];
    vbuffers[1] = mVertexBuffer[2];
    mImmediate->VSSetConstantBuffers(cbBindPoint, 1, cbuffers);
    mImmediate->IASetVertexBuffers(0, 2, vbuffers, strides, offsets);
    mImmediate->Draw(mNumVertices, mVertexOffset);

    // NOTE: The clearing here is conservative, to restore all state to what
    // it was before the Draw call.  You do not have to clear as long as you
    // set all the necessary state needed for the next draw call.
    ...
    offsets[1] = 0;
    cbuffers[0] = nullptr;
    mImmediate->IASetVertexBuffers(0, 1, vbuffers, strides, offsets);
    mImmediate->VSSetConstantBuffers(cbBindPoint, 1, cbuffers);
...
void Application::OnLeftMouseDown(int x, int y)
{
    ...
    memcpy(&mSaveTrackballMatrix[0][0], &mTrackballMatrix[0][0], 16 * sizeof(float));
...
void Application::ComputeWorldMatrix(int whichTriangle)
{
    float offset = 0.1f * (2.0f * whichTriangle - 1.0f);
    mWorldMatrix[whichTriangle][0][0] = 1.0f;
    mWorldMatrix[whichTriangle][0][1] = 0.0f;
    mWorldMatrix[whichTriangle][0][2] = 0.0f;
    mWorldMatrix[whichTriangle][0][3] = offset - 1.0f / 3.0f;
    mWorldMatrix[whichTriangle][1][0] = 0.0f;
    mWorldMatrix[whichTriangle][1][1] = 1.0f;
    mWorldMatrix[whichTriangle][1][2] = 0.0f;
    mWorldMatrix[whichTriangle][1][3] = offset - 1.0f / 3.0f;
    mWorldMatrix[whichTriangle][2][0] = 0.0f;
    mWorldMatrix[whichTriangle][2][1] = 0.0f;
    mWorldMatrix[whichTriangle][2][2] = 1.0f;
    mWorldMatrix[whichTriangle][2][3] = offset;
    mWorldMatrix[whichTriangle][3][0] = 0.0f;
    mWorldMatrix[whichTriangle][3][1] = 0.0f;
    mWorldMatrix[whichTriangle][3][2] = 0.0f;
    mWorldMatrix[whichTriangle][3][3] = 1.0f;
}
    ...
    for (int whichTriangle = 0; whichTriangle < 2; ++whichTriangle)
    {
        ...
        for (int r = 0; r < 4; ++r)
        {
            for (int c = 0; c < 4; ++c)
            {
                ...
                {
                    worldMatrix[r][c] +=
                        mTrackballMatrix[r][m] * mWorldMatrix[whichTriangle][m][c];
                }
            }
        }
        ...
        HRESULT hr = mImmediate->Map(mConstantBuffer[whichTriangle], 0,
            D3D11_MAP_WRITE_DISCARD, 0, &sub);
        if (SUCCEEDED(hr))
        {
            memcpy(sub.pData, &pvwMatrix[0][0], 16 * sizeof(float));
            mImmediate->Unmap(mConstantBuffer[whichTriangle], 0);
        }
    }
}
