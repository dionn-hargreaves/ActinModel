...
class ProjectedTextureEffect : public LightingEffect
{
public:
    // Construction.
    ...
    // After you modify the projector matrix, call the update to inform any listener
    // that the corresponding constant buffer has changed.
...
inline std::shared_ptr<Texture2> const& ProjectedTextureEffect::GetTexture() const
{
    return mTexture;
}

inline std::shared_ptr<SamplerState> const& ProjectedTextureEffect::GetSampler() const
{
    return mSampler;
}

inline std::shared_ptr<ConstantBuffer> const&
ProjectedTextureEffect::GetProjectorMatrixConstant() const
{
