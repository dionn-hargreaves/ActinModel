...
class GlossMapEffect : public LightingEffect
{
public:
    // Construction.
    GlossMapEffect(std::shared_ptr<ProgramFactory> const& factory, BufferUpdater const& updater,
...
inline std::shared_ptr<Texture2> const& GlossMapEffect::GetTexture() const
{
    return mTexture;
}

inline std::shared_ptr<SamplerState> const& GlossMapEffect::GetSampler() const
{
    return mSampler;
}
