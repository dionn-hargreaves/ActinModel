...
int main(int, char const*[])
{
    ...
    Window::Parameters parameters(L"CastleWindow", 0, 0, 800, 600);
    auto window = TheWindowSystem.Create<CastleWindow>(parameters);
    TheWindowSystem.MessagePump(window, TheWindowSystem.DEFAULT_ACTION);
    TheWindowSystem.Destroy<CastleWindow>(window);
    return 0;
}
...
void CastleWindow::CreateLights()
{
    ...
    mDLight = std::make_shared<Light>(true, false);
#else
    mDLight = std::make_shared<Light>(true, true);
#endif
...
void CastleWindow::InitializeFixedHeightRig()
{
