...
//----------------------------------------------------------------------------
// Public interface specific to DX11.
//----------------------------------------------------------------------------
...
bool DX11Engine::CreateSwapChain(HWND handle, UINT xSize, UINT ySize)
{
    ...
    LogAssert(SUCCEEDED(hr), "Failed to set private name, hr = " + std::to_string(hr));
#endif
...
#if defined(GTE_GRAPHICS_USE_NAMED_OBJECTS)
    hr = SetPrivateName(bb.depthStencilBuffer, "DX11Engine::mDepthStencilBuffer");
    LogAssert(SUCCEEDED(hr), "Failed to set private name, hr = " + std::to_string(hr));
#endif
...
bool DX11Engine::BindProgram(std::shared_ptr<ComputeProgram> const& program)
{
    ...
    if (hlslProgram)
    {
        auto cshader = hlslProgram->GetCShader();
        if (cshader)
        {
...
void DX11Engine::WaitForFinish()
{
    if (!mWaitQuery)
    {
        ...
        desc.MiscFlags = D3D11_QUERY_MISC_NONE;
        HRESULT hr = mDevice->CreateQuery(&desc, &mWaitQuery);
