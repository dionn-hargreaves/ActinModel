    ...
    mPlaneNormals(planes.size())
{
    ...
    for (unsigned int i = 0; i < numPlanes; ++i)
    {
        ...
        // for drawing the triangle mesh.  This prevents drawing attempts by another
        ...
        if (!positions)
        {
            ...
            mPlaneOrigins[i] = Vector4<float>::Zero();
            mPlaneNormals[i] = Vector4<float>::Zero();
            continue;
        }
        ...
        for (int j = 0; j < 3; ++j)
        {
            Vector3<float> temp = *reinterpret_cast<Vector3<float> const*>(positions + v[j] * vstride);
    ...
    VisibleSet const& visibleSet, PVWUpdater& pvwMatrices)
{
    ...
    // Get the current cull mode and reverse it.  Allow for models that are not
    ...
    for (unsigned int i = 0, reference = 1; i < numPlanes; ++i, ++reference)
    {
        ...
        // plane pixels occluded by other already drawn geometry are not drawn.
        // The stencil buffer value for pixels from plane i is (i+1).  The
        // stencil buffer is updated at a pixel only when the depth test passes
        // at that pixel (the plane pixel is visible):
        ...
        // plane in world coordinates and must be applied in the transformation
        // pipeline before the world-to-view matrix is applied; thus, we
        // insert the reflection matrix into the pipeline via SetPreViewMatrix.
        // Because the pvw-matrices are dependent on this, each time the
        // full transformation is computed we must update the pvw matrices in
        // the constant buffers for the shaders.  NOTE:  The reflected objects
        // will generate pixels whose depth is larger than that for the
        // reflecting plane.  This is not a problem, because we will later
        // draw the plane again and blend its pixels with the reflected object
        // pixels, after which the depth buffer values are updated to the plane
        // pixel depths.
#if defined(GTE_USE_MAT_VEC)
        Vector4<float> origin = plane->worldTransform * mPlaneOrigins[i];
        Vector4<float> normal = plane->worldTransform * mPlaneNormals[i];
#else
        Vector4<float> origin = mPlaneOrigins[i] * plane->worldTransform;
        Vector4<float> normal = mPlaneNormals[i] * plane->worldTransform;
#endif
        Normalize(normal);
        ...
        mReflectanceBlend->blendColor = { mReflectances[i], mReflectances[i], mReflectances[i], mReflectances[i] };
