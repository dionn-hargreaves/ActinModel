...
class GTE_IMPEXP DirectionalLightEffect : public LightingEffect
{
public:
    // Construction.  Set 'select' to 0 for per-vertex effects or to 1 for per-pixel effects.
    DirectionalLightEffect(std::shared_ptr<ProgramFactory> const& factory,
        BufferUpdater const& bufferUpdater, int select, std::shared_ptr<Material> const& material,
