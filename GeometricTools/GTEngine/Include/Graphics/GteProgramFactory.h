...
class GTE_IMPEXP ProgramFactory
{
public:
    // Abstract base class.
    ...
    // the 'csName' parameters in case the shader compiler needs this for
    // #include path searches.
    std::shared_ptr<ComputeProgram> CreateFromFile(
        std::string const& csFile);

    std::shared_ptr<ComputeProgram> CreateFromSource(
