...
// general-purpose 2D windowing layer.  The window resizing is disabled
// to avoid the complications of resizing the overlay texture and
// redrawing objects that were initially defined in the original window
// coordinates.

namespace gte
{

class GTE_IMPEXP Window2 : public Window
{
protected:
    // Abstract base class.  The window creation requires parameters.allowResize
    // to be 'false'; otherwise, the creation fails.
    Window2(Parameters& parameters);

public:
    // Display functions.  The OnResize function always returns 'false';
    // that is, you cannot resize the window.  The DrawScreenOverlay function
    // is called after the screen texture is drawn but before the swap-buffers
    // call is made.  This allows you to draw text or GUI elements on top of
    // the screen texture.
    ...
    // channel the most significant bits and red the least significant bits.
    // For example, (r,g,b,a) = (1,2,3,4) is 0x04030201.
    ...
    // ((x-xc)/a)^2 + ((y-yc)/b)^2 = 1, where xCenter is xc, yCenter is yc,
    // xExtent is a, and yExtent is b.
